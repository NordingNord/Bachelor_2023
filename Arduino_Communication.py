...
Voltage_Readings = {"V1":{},"V2":{},"V3":{},"V4":{},"V5":{}}
Newest_Reading = {"V1":0, "V2":0,"V3":0,"V4":0,"V5":0}
Voltage_Offsets = {"V1":0.47695312500000003, "V2":0.47695312500000003,"V3":0.47695312500000003,"V4":0.47695312500000003,"V5":0.47695312500000003}

Pressure_Readings = {"P1":{}, "P2":{},"P3":{},"P4":{},"P5":{}}
Current_Pressure = {"P1":0, "P2":0,"P3":0,"P4":0,"P5":0}
...
def Main_Samples(Samples):
    ...
    if __name__ == '__main__':
        ...
        Received_Sensors = {"S1":0, "S2":0,"S3":0 ,"S4":0 ,"S5":0}
        ...
        while Samples_Left > 0:
            ...
            if Received_Sensors["S1"] == 1 and Received_Sensors["S2"] == 1 and Received_Sensors["S3"] == 1:
                ...
                Samples_Left -= 1
            elif Received_Sensors["S4"] == 1 and Received_Sensors["S5"] == 1:
                Received_Sensors["S4"] = 0
                Received_Sensors["S5"] = 0
                print(Samples_Left)
                # Count down samples left
                Samples_Left -= 1
...
#                 S3_String += " "
            ...
            if Voltage_List[i] in Voltage_Readings["V4"]:
                S4_String += str(Voltage_Readings["V4"][Voltage_List[i]])
                S4_String += " "
            else:
                S4_String += str("0")
                S4_String += " "
            if Voltage_List[i] in Voltage_Readings["V5"]:
                S5_String += str(Voltage_Readings["V5"][Voltage_List[i]])
                S5_String += " "
            else:
                S5_String += str("0")
                S5_String += " "
...
#             S3_String = S3_String+"\n"
            if Voltage_List[i] in Voltage_Readings["V4"]:
                S4_String = str(Voltage_Readings["V4"][Voltage_List[i]])
                S4_String = S4_String.ljust(Padding_Short,' ')
            else:
                S4_String = str("0")
                S4_String = S4_String.ljust(Padding_Short,' ')
            ...
            if Voltage_List[i] in Voltage_Readings["V5"]:
                S5_String = str(Voltage_Readings["V5"][Voltage_List[i]])
                S5_String = S5_String.ljust(Padding_Short,' ')
            else:
                S5_String = str("0")
                S5_String = S5_String.ljust(Padding_Short,' ')
            S5_String = S5_String+"\n"
            ...
            Final_String = V_String+P_String+S4_String+S5_String
            # Write string
