...
Offsets = [0.007962348363469901,0.007962348363469901,0.007962348363469901,0.47695312500000003,0.47695312500000003]
...
def Read_Value():
    ...
    # Cibvert data into raw adc data
    ...
    Raw_Data = float(Raw_Data*My_Coefficient)
...
def Prepare_Data():
    ...
    return ADC_List, Voltage_List, Pressure_List
    
def Print_Data(ADC_List,Voltage_List,Pressure_List):
    # Initialize strings
    S1_String = ""
    S2_String = ""
    S3_String = ""
    ...
    for i in range(len(ADC_List)):
            ...
            if ADC_List[i] in ADC_Readings["S1"]:
                S1_String += str(ADC_Readings["S1"][ADC_List[i]])
                S1_String += " "
            ...
            if ADC_List[i] in ADC_Readings["S2"]:
                S2_String += str(ADC_Readings["S2"][ADC_List[i]])
                S2_String += " "
            else:
                S2_String += str("0")
                S2_String += " "
            ...
            if ADC_List[i] in ADC_Readings["S3"]:
                S3_String += str(ADC_Readings["S3"][ADC_List[i]])
                S3_String += " "
            else:
                S3_String += str("0")
                S3_String += " "
    ...
    for i in range(len(Voltage_List)):
        ...
        Final_String = ADC_String+V_String+P_String+S1_String+S2_String+S3_String
        # Write string
...
while True:
    ...
    # Prepare data
    DC_List,Voltage_List,Pressure_List = Prepare_Data()
    
    # Print the results
    Print_Data(DC_List,Voltage_List,Pressure_List)
